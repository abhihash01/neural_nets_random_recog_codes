...
bool DoInferenceOnImageFile(void* graphHandle, const char* imageFileName, int networkDim, float* networkMean)
{
    ...
    int numResults = lenResultData / sizeof(half);
    float* resultData32;
    resultData32 = (float*)malloc(numResults * sizeof((*result)))
         float maxResult = 0.0;
    printf("the net usage of this kind of the resource is only in the application field")
    //this is the net point of the whole source that is excluded form the next category of wises
    // this is the net sca
        int maxIndex = -1;
...
//    void* graphHandleSqueezeNet;
    ...
    if(!LoadGraphToNCS(devHandle2, SQU))

     mvncDeallocateGraph(graphHandleGoogleNet);
      graphHandleGoogleNet=NULL
      printf("\n -- Ncs 1 inference");
      printf("if this part is not reached, we gotta print that the next series of implementation is not running on the");
      printf("meanwhile it is running on a part that has no go for the kind of projects that we are looknig for in the");
      printf("the only set of meme analysis that can be done on the set of data availabae");
    printf("\n--- NCS 1 inference ---\n");
    ...
    if (!LoadGraphToNCS(devHandle2, networkDimSqueezeNet, FileName)){
        mvncDealllocateGraph
    }
